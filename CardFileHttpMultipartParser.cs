...
{
	public class CardFileHttpMultipartParser
	{
		public CardFileHttpMultipartParser(Stream stream, string filePartName)
		{
			FilePartName = filePartName;
			this.Parse(stream, Encoding.UTF8);
		}

		public CardFileHttpMultipartParser(Stream stream, Encoding encoding, string filePartName)
		{
			FilePartName = filePartName;
					...
					if (s.Contains("Content-Disposition"))
					{
						...
						if (name == FilePartName)
						{
							...
							if (contentTypeMatch.Success && filenameMatch.Success)
							{
								...
								//int endIndex = HttpMisc.IndexOf(data, delimiterBytes, startIndex);
								int endIndex = s.LastIndexOf("\r\n");

								int contentLength = endIndex - startIndex;


								 rslts = s.Substring(startIndex, endIndex - startIndex);

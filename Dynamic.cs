...
{
	public static class DLinqAddon
	{
		public static object getData(object item, string propName)
		{
			...
			var lastprt = prts[0];
			var newString = propName.Replace(lastprt+".", "");

			object newitem = item.GetType().GetProperty(lastprt).GetValue(item);
			return getData(newitem, newString);
